...
from rlrecs.agents.trainer import BCQTrainer
...
def run_offline(eval:Optional[bool]=False):
    config = configparser.ConfigParser()
    file_path = "/home/inoue/work/RLRecs/config/bcq.conf"
    config.read(file_path)
    hidden_dim = int(config["AGENT"]["HIDDEN_DIM"])
    seq_len=int(config["ENV"]["SEQ_LEN"])
    embed_dim=int(config["AGENT"]["EMBED_DIM"])
    ...
    max_iteration = int(config["AGENT"]["MAX_ITERATION"])
    batch_size = int(config["AGENT"]["BATCH_SIZE"])
    eta = float(config["AGENT"]["ETA"])
    lam = float(config["AGENT"]["LAM"])
    
    logger = create_logger("BCQ")
    ...
    if eval:
        ...
        agent.load("/home/inoue/work/RLRecs/models/RC15/bcq")
        ...
        trainer.agent.save("%s/work/RLRecs/models/RC15/bcq"%(HOME))
