...
from rlrecs.agents.trainer import QRDQNTrainer
from rlrecs.agents.models import QRDQN
...
def run_offline(eval):
    config = configparser.ConfigParser()
    file_path = "/home/inoue/work/RLRecs/config/qrdqn.conf"
    config.read(file_path)
    hidden_dim = int(config["AGENT"]["HIDDEN_DIM"])
    seq_len=int(config["ENV"]["SEQ_LEN"])
    embed_dim=int(config["AGENT"]["EMBED_DIM"])
    ...
    max_iteration = int(config["AGENT"]["MAX_ITERATION"])
    batch_size = int(config["AGENT"]["BATCH_SIZE"])
    num_quantiles = int(config["AGENT"]["NUM_QUANTILES"])
    
    logger = create_logger("QRDQN")
    ...
    if eval:
        ...
        agent.load("/home/inoue/work/RLRecs/models/RC15/qrdqn")
        ...
        trainer.agent.save("%s/work/RLRecs/models/RC15/qrdqn"%(HOME))
