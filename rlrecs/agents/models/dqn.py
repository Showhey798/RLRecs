...
def greedy_action(
    ...
    recommend_items = jnp.argmax(qvalue, axis=1)
...
class DQN(BaseAgent):
    ...
    def recommend(self, inputs:Tuple[np.ndarray], click_masks:Optional[np.ndarray]=None):
        state, feedback = inputs
        if click_masks is None:
            click_masks = np.zeros((state.shape[0], self.num_items), dtype=np.float32)
